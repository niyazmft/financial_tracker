  ...
  <div>
    ...
    <Card class="mb-8">
      <template #content>
        ...
        <ProgressBar
          :value="Math.round(savings.totalProgress)"
          :show-value="false"
          style="height: 1rem"
        />
        <p class="text-sm text-text-sub text-right mt-2">
          {{ Math.round(savings.totalProgress) }}% of total target ({{ formatCurrency(savings.totalTarget) }})
        </p>
      </template>
    </Card>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        v-if="savings.goals.length === 0 && !isLoadingSavings"
        class="col-span-full text-center py-12 text-text-sub"
      >
        No savings goals yet. Add one to get started!
      </div>
      <Card
        v-for="goal in savings.goals"
        :key="goal.Id"
      ...
      >
        ...
        <template #content>
          <ProgressBar
            ...
            :show-value="false"
            :severity="goal.is_fully_funded ? 'success' : 'primary'"
            class="mb-3"
            style="height: 8px"
          />
          ...
          <p
            ...
            class="text-xs text-text-mute text-right mt-2"
          >
    ...
    <!-- Add Goal Dialog -->
    <Dialog
      v-model:visible="showAddGoalModal"
      header="Add New Savings Goal"
      modal
      :style="{ width: '450px' }"
    >
