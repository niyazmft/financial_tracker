<template>
    <div class="relative z-50">
        <Menubar 
            :model="items" 
            class="border-none bg-card-bg px-4 py-2 shadow-soft sticky top-0 w-full rounded-none border-b border-border-base"
            :pt="{
        ...
        >
            ...
            <template #end>
                <div class="flex items-center gap-2">
                    ...
                    <Avatar v-if="authStore.user" :image="userPhotoURL" shape="circle" class="cursor-pointer" @click="toggleUserMenu" />
...
const allItems = [
    ...
    { label: 'Installments', icon: 'pi pi-calendar-plus', route: '/installment_plans' },
    { label: 'Spending', icon: 'pi pi-chart-pie', route: '/spending_analysis' },
    { label: 'Budgets', icon: 'pi pi-wallet', route: '/budget_manager' },
    { label: 'Subscriptions', icon: 'pi pi-sync', route: '/subscriptions' },
    { label: 'Reports', icon: 'pi pi-file-pdf', route: '/reports' }
];

const items = computed(() => {
    // Breakpoints for optimal responsive layout:
...
const userPhotoURL = computed(() => {
    return authStore.user?.photoURL || 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y';
});
