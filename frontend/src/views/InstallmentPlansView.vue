<template>
  <div class="p-4 md:p-8 max-w-7xl mx-auto">
    ...
    <Tabs v-model:value="activeTab">
      ...
      <TabPanels class="bg-transparent px-0 py-6">
        ...
        <TabPanel value="upcoming">
          ...
          >
            <div
              v-for="group in upcomingPayments"
              :key="group.groupKey"
            >
              <h3 class="text-xl font-bold mb-6 sticky top-[120px] bg-app-bg/80 backdrop-blur-sm py-2 z-10 border-b border-border-base">
                {{ group.monthName }}
              </h3>
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card
                  v-for="payment in group.installments"
                  :key="payment.Id"
                ...
                >
                  <template #title>
                    <div class="flex justify-between items-start">
                      <span
                        v-tooltip="payment.planName"
                        class="font-bold truncate max-w-[70%]"
          ...
          >
            ...
            >
              <template #content>
                <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
                  <div class="flex-1 min-w-[200px]">
                    ...
                    <p class="text-sm text-text-sub">
                      Total: <b>{{ formatCurrency(plan.totalAmount) }}</b> • {{ plan.installmentCount }} Installments
                    </p>
                  </div>
                  <div class="flex items-center gap-4">
                    <div class="text-right">
                      <p class="text-xs font-bold uppercase tracking-widest text-text-mute">
                        Progress
                      </p>
                ...
                <div
                  v-if="expandedPlans[plan.itemId]"
                  class="mt-6 border-t border-border-base pt-4 overflow-x-auto"
                >
                  <DataTable
                  ...
                  >
                    ...
                    >
                      <template #body="{ data }">
                        {{ formatCurrency(data.installment_payment) }}
                      </template>
                    </Column>
                    <Column
                      field="paid"
                    ...
                    >
                      <template #body="{ data }">
                        ...
                        <span
                          v-else
                          class="text-xs text-text-mute italic"
                        >Completed</span>
                      </template>
                    </Column>
                  </DataTable>
    ...
    <Dialog
      ...
      header="Add New Installment Plan"
      modal
      :style="{ width: '50rem' }"
    >
      <div class="flex flex-col gap-6 py-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-2">
            ...
            <InputNumber
              id="add-count"
              v-model="addForm.installments_count"
              :min="1"
            />
          </div>
          <div class="flex flex-col gap-2">
        ...
        <div
          v-if="schedulePreview.length > 0"
          class="border-t border-border-base pt-4"
        >
            ...
            >
              <template #body="{ data }">
                <input
                  ...
                  type="date"
                  class="bg-transparent border-none p-0 text-sm focus:ring-0"
                >
              </template>
            </Column>
            <Column
            ...
            >
              <template #body="{ data }">
                <input
                  ...
                  type="number"
                  step="0.01"
                  class="bg-transparent border-none p-0 text-sm font-bold focus:ring-0 w-24 text-right"
                >
              </template>
            </Column>
          </DataTable>
          <div class="text-right mt-2 font-bold">
            Total: {{ formatCurrency(scheduleTotal) }}
          </div>
        </div>
      </div>
      <template #footer>
      ...
      <div class="flex flex-col gap-4 py-4">
        ...
        >
          ...
          >
            <template #body="{ data }">
              <input
                v-model.number="data.installment_payment"
                type="number"
                step="0.01"
                class="bg-transparent border-none p-0 text-sm font-bold focus:ring-0 w-24"
              >
            </template>
          </Column>
          <Column
          ...
          <Column header="Action">
            <template #body="{ data }">
              <Button
                ...
                text
                rounded
                @click="confirmDeleteInstallment(data.Id)"
              />
            </template>
          </Column>
        </DataTable>
